*Purpose: this is creating plots of the simulation data only (have now replicated this in python)


*cd "C:\Users\wb488473\WBG\Data analysis to inform Global Water Operations - WB Group - Documents\Zim COVID - via Teams\08_Data\files for Sarah\outputs_gitignored\"

cap cd "C:\Users\wb488473\OneDrive - WBG\Documents\GitHub\Disease-Modelling-SSA\data\output\"

tempfile temp1

*********************import file  and assign local*********************

*use district level dataset
**use "processed/model_outputs_dist_level.dta", replace
**use "processed/model_outputs_dist_level_p016_120_18062021", replace

local filename direct_output/sensitivity_analysis/exportMe2439_03_180_20GB.txt
local kind 2439_03_180_20GB

import delimited `filename', encoding(ISO-8859-2) clear

split myid, p("d_")
drop v9 myid1 myid
rename myid2 dist_no
rename time day
destring dist_no, replace

*drop after day 80
**drop if day >80

*add in district names
order dist_no
sort dist_no

*change var names
rename metric_new_cases_sympt new_sympt
rename metric_died_count died_count
rename metric_new_critical new_crit
rename metric_new_deaths new_deaths
rename metric_new_hospitalized new_hosp

save `temp1', replace

import delimited "../processed/district_names.csv", encoding(ISO-8859-9) clear
sort dist_no
 
merge 1:m dist_no using `temp1'
drop _merge

*sort ready for next stage
sort dist_no day


*******************************CUMULATIVE DATA**************************



*create cumulative totals for case numbers & deaths (this needs correcting)
bysort district (day): gen cumul_cases=sum(new_sympt)
scatter cumul_cases day, title("District predictions: Cumulative symptomatic")
graph export "plots/district/m_cumul_cases_dist_`kind'.png", replace

*don't need to make cumul_deaths as we have died_count
scatter died_count day, title("Dist level predictions: Cumulative deaths")
graph export "plots/district/m_cumul_deaths_dist_`kind'.png", replace

* create cumulative for hosp and crit
bysort dist_no (day): gen cumul_hosp=sum(new_hosp)
scatter cumul_hosp day, title("District predictions: Cumulative Hospitalizations")
graph export "plots/district/m_cumul_hosp_dist_`kind'.png", replace

scatter cumul_hosp day, title("District predictions: Cumulative Hospitalizations") msize(small)
graph export "plots/district/m_cumul_hosp_dist_scatter_`kind'.png", replace


bysort district (day): gen cumul_crit=sum(new_crit)
scatter cumul_crit day, title("District predictions: Cumulative Critical Cases")
graph export "plots/district/m_cumul_crit_dist_`kind'.png", replace


*******************************COMPARE DISTRICTS COLLAPSE **************

preserve 
*when is the first occasion in each district where the value of cumul_cases is at least 1 (https://www.stata.com/support/faqs/data-management/first-and-last-occurrences/) 

by district(day), sort: gen byte first = sum(inrange(cumul_cases, 1,.)) == 1  & sum(inrange(cumul_cases[_n - 1],1,.)) == 0  
 
 *create a variable to indicate which day that was 
 
gen day_first_case = day if first ==1 

*collapse at district level to keep the total number of cases, hospitalizations, critical and deaths by district, and the day of the first case by district 
collapse (max) cumul_cases cumul_hosp cumul_crit died_count day_first_case, by(dist_no district district_name_shpfile)

save "../processed/model_outputs_collapsed_dist_`kind'.dta", replace
export excel "../processed/model_outputs_collapsed_dist_`kind'.xlsx", first(var) replace


*make graphs of which district came first
*and the number of cases in each district on that day
scatter day_first_case day, mlabel(district_name_shpfile) msize(.2cm) 

keep if day_first_case !=.
graph bar cumul_cases, over(district_name_shpfile, sort(day_first_case)) nofill title("no. cases on first day of cases in that district")
graph export "plots/district/cases_districts_at_start_`kind'.png", replace


graph bar day_first_case, over(district_name_shpfile, sort(day_first_case)  lab(angle(45) labsize(vsmall))) nofill title("order of cases appearing by district") 
graph export "plots/district/first_days_districts_`kind'.png", replace


twoway scatter cumul_cases day_first_case, mlabel(district_name_shpfile) xtitle("day of first case in district") ytitle("cumulative cases in district on that day") title("First cases in districts")

graph export "plots/district/first_day_cases_district_`kind'.png", replace

restore





*******************************CASE DATA*******************************


preserve
*just cases by district by days
keep day dist_no district new_sympt
xtset dist_no day
set scheme cleanplots, perm

*to be improved graphed cases by district overlaid
xtline new_sympt, overlay title("Daily cases by district") scheme(cleanplots)  legend(size(tiny))

graph export "plots/district/m_new_sympt_district_day_overlay_`kind'.png", replace

*can include above but for some reason colours are not there
*refer here for more tips https://medium.com/the-stata-guide/covid-19-visualizations-with-stata-part-2-customizing-color-schemes-206af77d00ce


*graphs for each district individually as a panel 
twoway (line new_sympt day), by(district) 
graph export "plots/district/m_new_sympt_district_day_`kind'.png", replace

*reshape the data to play with in excel, r or python for better visuals
rename new_sympt ns_
reshape wide ns_, i(district dist_no) j(day)
sort dist_no
export excel "../processed/model_outputs_daily_cases_dist_reshape_`kind'.xlsx", first(var) replace



restore

*******************************HOSP & CRIT DATA************************

*to be improved graphed cases by district overlaid
sort dist_no day
xtset dist_no day
xtline new_hosp, overlay title("Daily Hospitalizations by district") scheme(cleanplots)  legend(size(tiny))

xtset dist_no day
xtline new_crit, overlay title("Daily Critical by district") scheme(cleanplots)  legend(size(tiny))

*******************************DEATH DATA*******************************

preserve
*just deaths by district by days
keep day dist_no district new_deaths died_count


sort dist_no day
xtset dist_no day
set scheme s1color


*to be improved graphed cases by district overlaid
xtline new_deaths, overlay title("Daily deaths by district") scheme(cleanplots)  legend(size(tiny))

*graphs for each district individually as a panel 
twoway (line new_deaths day), by(district) 
graph export "plots/district/m_new_deaths_district_day_`kind'.png", replace

*graph for cumulative deaths for each district overlaid
xtline died_count, overlay title("Cummulative deaths by district") scheme(cleanplots)  legend(size(tiny))
graph export "plots/district/m_cumul_deaths_district_day_`kind'.png", replace


// *reshape the data 
// rename died died_
// reshape wide died_, i(district dist_no) j(day)
// sort dist_no
// export excel "processed/model_outputs_daily_deaths_dist_reshape_`kind'.xlsx", first(var)replace

restore

********************* CHECKING AGAINST PYTHON 
tempfile temp1

import delimited "C:\Users\wb488473\OneDrive - WBG\Documents\GitHub\Disease-Modelling-SSA\data\preprocessed\line_list\true.txt", clear 

sort districtid

save `temp1', replace

import delimited "C:\Users\wb488473\OneDrive - WBG\Documents\GitHub\Disease-Modelling-SSA\data\output\direct_output\sensitivity_analysis/ema_appended.csv", clear

sort districtid
merge m:1 districtid using `temp1'

//check complete. Python merge code now works. Output file for this is ema_plus_gold.csv 
